...
const downloadPDF = ({ situation, preparation, empathy, phrasing }) => {
  ...
  const drawBlock = (labelText, bodyText, highlighted = false) => {
    ...
    // Set body font before splitting so dimensions match
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    const lines = doc.splitTextToSize(bodyText, innerW);
    const blockH = 5 + 5 + lines.length * lineHeight + 4; // label row + gap + body + padding
    checkPage(blockH + 4);
    ...
    doc.text(lines, margin + 4, y + 11, { lineHeightFactor: lineHeight / 3.528 });
  ...
  if (empathy.tone) {
    ...
    drawBlock("Tone", `${empathy.tone.charAt(0).toUpperCase() + empathy.tone.slice(1)} — ${toneDesc}`);
  }
  if (empathy.emotions?.length) drawBlock("Emotions to Bring", empathy.emotions.join(" · "));
  const feelingVal = Array.isArray(situation.feeling) ? situation.feeling.join(" · ") : situation.feeling;
  if (feelingVal) drawBlock("Going in Feeling", feelingVal);
  y += 4;

  // Pillar 3
  drawSectionLabel("Pillar 3", "Your Openers");
  if (phrasing.instinct) drawBlock("Your Instinct Was", `"${phrasing.instinct}"`);
  if (phrasing.iStatementText) drawBlock("I-statement", `"${phrasing.iStatementText}"`, true);
  if (phrasing.gottmanText) drawBlock("Soft Opening", `"${phrasing.gottmanText}"`, true);
  if (phrasing.framingText) drawBlock("Framing", `"${phrasing.framingText}"`, true);
  ...
  doc.text(new Date().toLocaleDateString(), W - margin, pageH - 7, { align: "right" });
